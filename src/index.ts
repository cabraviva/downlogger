...
export default class Logger {
      ...
      middleWare: (req: any, res: any, next: any) => {
        ...
        onFinished(res, () => {
          ...
          self
            .inContext('WebServer')
            .info(
              `${payload.method} ${payload.url} - ${payload.response.statusCode} - ${payload.response.responseTimeInMs}ms FROM ${ip}`,
            )
        })
  ...
  print(...messages: unknown[]): void {
    ...
    this.files.forEach((file) => {
      fs.appendFileSync(
        file,
        `[${humanDate()}: CONSOLE OUTPUT] ${messages.join(' ')}\n`,
      )
    })
  }
  ...
  pipe(file: string): void {
    ...
    this.printrf(
      `-- Total Memory: ${os.totalmem()} bytes (${(os.totalmem() / 1024 / 1024 / 1024).toFixed(2)} GB)`,
    )
    this.printrf(
      `-- Free Memory: ${os.freemem()} bytes (${(os.freemem() / 1024 / 1024 / 1024).toFixed(2)} GB)`,
    )
    this.printrf('------------------------------------------------------------')
  }
}
